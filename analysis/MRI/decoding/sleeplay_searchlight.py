...
save_dir_files = os.path.join(save_dir, 'files')
...
# all_sub = [sub] # ['sub-01','sub-02']
nses = ['ses-01', 'ses-02', 'ses-03', 'ses-04'] #'ses-01',*'ses-02'*, *'ses-03'*, 'ses-04' 
if sub == 'sub-03':
    nses = ['ses-02', 'ses-03', 'ses-04'] 
cue_types = ['enc', 'ret', 'enc&ret'] # , 'enc2ret' # where classifiers are trained/tested on 
### --> SET CUE TYPE TO DATA YOU WANT TO TRAIN/TEST CLASSIFIER ON
# cue_type= cue_types[1] #0=enc, 1=ret, 2=enc2ret, 3=enc&ret
TR = 1.25
...
def pickNprep_TRs(func_data, time_idx, cat_labels, runs, run_vector, HRF):    
   ...
   # del_TRs = 
    ...
    for irun in range(0,len(runs)):
        ...
        #     label_idx = np.argwhere(cat_labels==i).flatten('F')
        #     run_tmp = run_vector[label_idx]
        #     TR_tmp = TR_idx[label_idx]
        #     label_TR = TR_tmp[run_tmp == irun+1].astype(int)
        #     label_TR = np.delete
            
        # label_idx = cat_labels[run_vector == irun+1 and cond_labels[0]]
...
#%% PLOTTING MASK AND TIME-SERIES TO CHECK
        
        plot=False

        # =====================================================================
        # PLOT NIFTI MASKER AND/OR VOXEL TIME SERIES BEFORE/AFTER PRE-PROCESSING
        # =====================================================================
        
        if plot:
            # plot func mask on top of func scan data 
            plot_masker = masker[0].fit(func_data[0])
            # get func mask image
            mask_img = plot_masker.mask_img_
            # calculate mean image for the background
            mean_func_img = image.mean_img(func_data[0])
            plotting.plot_roi(mask_img, mean_func_img, display_mode="y", cut_coords=4, title="Mask")
            report = plot_masker.generate_report().open_in_browser()
            
            #plot voxel time-series of X middle voxels before and after pre-processing
            raw=False
            vox=2
            ...
            if raw: 
                plt_name = f"{vox}_Raw_voxel_timeseries_{sub}_{ses}_run01"
                plt.title(plt_name)
                plt.plot(raw_timeseries[:150, func_preproc[0].shape[1]//2:(func_preproc[0].shape[1]//2)+vox])
            else:
                plt_name = f"{vox}_Pre-processed_voxel_timeseries_{sub}_{ses}"
                plt.title(plt_name)
                plt.plot(func_preproc[0][:150, func_preproc[0].shape[1]//2:(func_preproc[0].shape[1]//2)+vox])
...
#%% BEGIN SEARCHLIGHT
        ...
        t1 = time.time()
        total = ((t1-t0)/60)/60
        print(f"Total run time {sub}: " + str(total) + ' hours') 
...
#%% SAVE OUTPUT MAPS
        ...
        sl_map_savename =  f"SearchLightMap_{sub}_{ses}_{cue_type}.nii"
        nb.save(searchlight_img, os.path.join(save_dir_files, sl_map_savename))

# ###### FROM CHATGPT
# from sklearn.model_selection import BaseCrossValidator
# import numpy as np

# class CustomLeaveOneGroupOut(BaseCrossValidator):
...
#         for train_group, test_group in zip(self.train_data, self.test_data):
#             train_indices = np.where(np.isin(groups, train_group))[0]
#             test_indices = np.where(np.isin(groups, test_group))[0]
#             yield train_indices, test_indices

# # Example usage:
# train_data = [train_data_1, train_data_2, ...]  # List of training datasets
# test_data = [test_data_1, test_data_2, ...]     # List of testing datasets

# # Assume you have the same number of groups in train_data and test_data
# # You can extract the group labels from train_data or test_data, as they should be the same
# group_labels = [group_labels_1, group_labels_2, ...]  # List of group labels

# custom_cv = CustomLeaveOneGroupOut(train_data, test_data)

# # Now, you can pass custom_cv to the cv parameter of your classifier
