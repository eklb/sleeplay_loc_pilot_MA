...
PATH_SCRIPT="${PATH_REPO}/b_loc_v2/analysis/MRI/decoding/sleeplay_searchlight_shuff.py"
...
for SUB in ${SUB_LIST[@]}; do
    ...
    for SES in ${SES_LIST[@]}; do
        ...
        for REP in $(seq 49 ${SEQ_LEN}); do
    
            #echo "sub-${SUB}"
            # get the subject number with zero padding:
        	#SUB_PAD=$(printf "%02d\n" ${SUB})
        	# Get job name
        	JOB_NAME="sleeplay_searchlight_sub-${SUB}_ses-${SES}_permute_enc" #SUB_PAD
        	# Create job file
        	...
        	# write to log folder
        	echo "#SBATCH --output ${PATH_LOG}/log_searchlight_sub-${SUB}_ses-${SES}_-enc.%j_perm-${REP}.out" >> job.slurm
        	...
        	echo "python3 ${PATH_SCRIPT} '${SUB}' '${SES}' '${REP}'" >> job.slurm 
