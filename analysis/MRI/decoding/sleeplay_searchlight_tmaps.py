...
all_sub = ['sub-02']#['sub-01', 'sub-02', 'sub-03']
...
perm_obj_enc, perm_obj_ret = np.empty((5, nrep_perm), dtype=object), np.empty((5, nrep_perm), dtype=object)
perm_sc_enc, perm_sc_ret = np.empty((6, nrep_perm), dtype=object), np.empty((6, nrep_perm), dtype=object)
            ...
            for ises, ses in enumerate(nses):
                ...
                for map_idx, file_name in sorted(enumerate(os.listdir(permuted_map_dir))):
                    if enc_type == 'obj' and cond  == 'enc':
                        perm_obj_enc[isub+ises,map_idx] = os.path.join(permuted_map_dir, file_name)
                    elif enc_type == 'obj' and cond == 'ret':
                        perm_obj_ret[isub+ises,map_idx] = os.path.join(permuted_map_dir, file_name)
                    elif enc_type == 'sc' and cond == 'enc':
                        perm_sc_enc[isub+ises,map_idx] = os.path.join(permuted_map_dir, file_name)
                    elif enc_type == 'sc' and cond == 'ret':
                        perm_sc_ret[isub+ises,map_idx] = os.path.join(permuted_map_dir, file_name)
                    # break
        isub += 2
...
for key, perm_map in zip(perm_mean_keys, all_permutes):
    for idc, paths in enumerate(perm_map.T):
        perm_map_ = [image.load_img(path) for path in paths if path is not None]
        if post_smooth:
...
for map_idx, (obs_key, perm_key, pmap_key) in enumerate(zip(obs_mean_keys, perm_mean_keys, pmap_keys)):
    for vox_idx, val in np.ndenumerate(obs_means[obs_key]):
        p_count = 0
        for perm_map in perm_means[perm_key]:
            if perm_map[vox_idx] > val:
                p_count += 1
                
        p_val = p_count/len(perm_means[perm_key])
    ...
    p_val_img = new_img_like(mean_func_data, p_ma)
    #save pmap
    nb.save(p_val_img, os.path.join(save_dir, 'pmap_' + obs_key + 'smooth.nii'))
...
if thresh:
    for idx, (pmap_key, obs_key) in enumerate(zip(pmap_keys, obs_mean_keys)):
        ...
        p_val_img = new_img_like(mean_func_data, p_ma)
        #save tmap
        nb.save(p_val_img, os.path.join(save_dir, 'tmap0_' + obs_key + 'smooth.nii'))
...
if plot_only:
    ...
    fsaverage
    # texture = surface.vol_to_surf(all_tmaps[tmap_key], (fsaverage.pial_right, fsaverage.pial_left))
    template = load_mni152_template(resolution=2)
    
    txt_obj_enc = surface.vol_to_surf(all_observed[0], fsaverage.pial_right)
    txt_obj_ret = surface.vol_to_surf(all_observed[1], fsaverage.pial_right)

    fig = plotting.plot_surf_stat_map(
    fsaverage.infl_right, txt_obj_enc, hemi='right',
    title='Accuracy map, obj enc right hemisphere', cmap='binary',
    threshold=0.12, bg_map=curv_right_sign, engine='matplotlib'
    )
    
    fig.savefig(os.path.join(save_dir, 'acc_map_surf_obj_enc'))
    
    fig = plotting.plot_surf_stat_map(
    fsaverage.pial_left, txt_obj_ret, hemi='left',
    title='Accuracy map, obj ret left hemisphere', cmap='Blues',
    threshold=0.11, bg_map=curv_left_sign, engine='matplotlib'
    )
   
    
    fig.savefig(os.path.join(save_dir, 'acc_map_surf_obj_ret'))

    plotting.plot_img_on_surf(all_observed[0],
                          views=['lateral'],
                          hemispheres=['right'],
                          colorbar=True,
                          threshold=0.12,
    ...
    fig.savefig(os.path.join(save_dir, 'acc_map_MNI_obj_enc'))
    ...
    fig.savefig(os.path.join(save_dir, 'acc_map_MNI_obj_ret'))
    ...
    for file in os.listdir(save_dir):
        if file.startswith('tmap') and file.endswith('smooth.nii'):
    ...
    # all_tmaps = dict()
    # for file in load_tmap:
    #     if file[10:-16] == 'obj_enc':
    #         all_tmaps['tmap_obj_enc'] = file
    #     elif file[10:-16] ==  'obj_ret':
    #         all_tmaps['tmap_obj_ret'] = file
    #     elif file[10:-16] == 'sc_enc':
    #         all_tmaps['tmap_sc_enc'] = file
    #     elif file[10:-16] == 'sc_ret':
    #         all_tmaps['tmap_sc_ret'] = file
