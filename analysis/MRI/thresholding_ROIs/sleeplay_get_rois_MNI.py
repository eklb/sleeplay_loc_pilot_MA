...
# ========================================================================
sub_all = ['sub-02'] #['sub-01', 'sub-02', 'sub-03']
ses_all = ['ses-01'] # 'ses-01', 'ses-02', 'ses-03', 'ses-04'
...
for isub, sub in enumerate(sub_all):
    ...
    for ises, ses in enumerate(ses_all):
        ...
        path_output_f = os.path.join(path_output_files, 'MNI')
        path_output_i = os.path.join(path_output_info, 'MNI_Info')
        ...
        for space in spaces:
                ...
                for run in np.arange(1,RUNS+1):  
                    ...
                    [print(np.sum(roi)) for roi in [VIS, MTL, HPC, HPPHC, PHPC, ERC, PPC, PREC, IPL, DLPFC, OTC, MOT]] #VIS, MTL, HPC, HPPHC, PHPC, ERC, PPC, PREC, IPL, MOT
                    ...
                    for thresh in [.8]: #.0, .3, .5,
                       ...
                       #VIS, MTL, HPC, HPPHC, PHPC, ERC, PPC, PREC, IPL, DLPFC, OTC, MOT
                        for roi, roi_name in zip([VIS, MTL, HPC, HPPHC, PHPC, ERC, PPC, PREC, IPL, DLPFC, OTC, MOT],
                                ['VIS', 'MTL', 'HPC', 'HPPHC', 'PHPC', 'ERC', 'PPC', 'PREC', 'IPL', 'DLPFC', 'OTC','MOT']):
        ...
        np.savetxt(os.path.join(path_output_i,"ROIsDropout"+sub+'_'+ses+".csv"), np.vstack(sub_data), delimiter=',',fmt = '%s')     
